...
| 数据类型 | 系统目录 | 用户目录 |
| --- | --- | --- |
| 出征计划 | `resource/system_battle_plans/` | `resource/user_battle_plans/` |
| 舰队方案 | `resource/system_team_plans/` | `resource/user_team_plans/` |
...
旧计划可能在 `fleet_presets` 中直接包含舰队内容。转换功能会：
...
多文件旧计划转换会先检查冲突，再执行写入，降低只生成一半文件的风险。
...
| `resource/user_team_plans/*.yaml` | 用户舰队方案 |
| `resource/user_battle_plans/*.yaml` | 用户出征计划 |
...
当前实现规模较大，后续可在不改变 IPC 契约的前提下拆分为独立模块；
本次 PR 优先保证完整行为和数据一致性。
