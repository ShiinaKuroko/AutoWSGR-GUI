...
Windows 安装产物包含 `resources/.autowsgr-install-manifest.json`，以相对路径和
SHA-256 记录当前版本拥有的程序文件。安装器在调用兼容版旧卸载器前暂存前后两个
版本的清单；旧卸载器只校验清单，不在新版文件落盘前删除程序文件。electron-builder
完成新包解压后，安装器先比较两份清单，只校验新增和哈希变化的目标文件，再将旧清单
独有的下架文件移动到同盘备份区。移动中途失败会逆序恢复；全部成功后才删除备份。
未登记文件、`log`、`logs`
和 `python/site-packages` 不进入清理范围，目录也只在没有其他文件时移除。已下载
安装包与 blockmap 位于 electron-updater 用户缓存，不属于安装目录清单。

清单哈希用于区分新增、修改和未变化文件，并验证最终产物。当前 NSIS 仍使用
electron-builder 的标准整包解压，因此物理安装过程会写入新包中的全部程序文件；
这里的增量边界是保留安装目录、只清理下架的受管文件、绝不清理清单外文件，不应
描述成二进制补丁或只写发生变化的文件。

首次从未携带清单的旧版本升级到兼容版时，仍由旧版卸载器执行原有完整替换。兼容版
写入 schema v2 清单后，后续升级才进入按程序文件所有权清理的路径。清单缺失、路径
越界、重解析点、新增或修改文件哈希不符、清理失败时必须中止升级，不能回退为整目录
删除。
