...
- 当前仓库是个人 Fork。`main` 只作为同步主库代码的中转分支，不直接开发功能；同步上游后再更新个人 Fork 的 `origin/main`。
- `ShiinaKuroko` 是主力开发分支，代表个人 Fork 的最新有效代码。功能实现、正式自动化测试和必要生成入口必须先合入该分支并完成验证，再由该分支执行普通 push。
- 本地工作分支可按任务自由创建。临时实验、一次性验证和调试代码不得合入；与功能对应、需要长期保留的自动化测试属于有效改动，应与实现一起合入 `ShiinaKuroko`。
- 临时 PR 分支是唯一允许额外推送的功能分支，格式为 `feat/<功能>-PR` 或 `fix/<功能>-PR`；Git 分支名不得使用反斜杠。创建下一条 PR 前必须检查上一条临时 PR 的状态，确认代码已经合入目标分支后，删除对应的本地和远端 PR 分支；未合入或状态不明确时不得删除。
...
- push 前以当前 `origin/ShiinaKuroko` 为指针创建远端不可移动备份 `backup/YYYYMMDD-<short-sha>`。日期精确到日，短 SHA 用于区分同一天的多次 push；必须先确认远端备份存在且提交正确，再 push 新代码。
- 远端 backup 分支最多保留两个。创建第三个备份并验证成功后，按名称日期和提交时间删除最旧的一个；不得移动、复用或覆盖已有 backup 分支。
