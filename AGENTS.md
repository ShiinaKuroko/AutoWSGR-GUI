...
- Stable 通道使用主库 GUI 与主库 AutoWSGR 后端；Alpha 通道使用个人仓库 GUI 与个人仓库 AutoWSGR 后端。managed 后端在 GUI 重启后按所选通道切换。
- GUI 必须明确提示：预览版可以快速体验当前正在开发的新功能，但可能遇到尚未修复的 Bug。
...
- `origin` 是个人仓库，Agent 可以按用户明确指令提交、打包和发布；主库只允许读取、同步和提交 PR，禁止直接 push、创建 Tag 或发布 Release。
- 主库发布版本是个人仓库编排版本号的外部基线，不代表 Agent 获得主库写入或发布权限。

#### 打包前版本核对

- GUI 正式打包只能在 `ShiinaKuroko` 分支执行，并沿用主库维护者当前发布工作流和产物规则。执行前必须确认 `git branch --show-current` 为 `ShiinaKuroko`；分支不符时停止，不得在 Sakuya、main 或临时 PR 分支打包。
- 修改 `package.json` 或 `package-lock.json` 版本、执行 `npm run dist`、创建或推送 Tag、创建 GitHub Release 或上传安装包前，必须先完成主库版本核对。
- 核对时必须读取个人仓库与主库的最新分支、Tags 和 Releases，并检查主库目标分支 `package.json` 的当前版本；未配置主库 remote 时，通过主库 GitHub 仓库执行只读核对，不得因此跳过。
- Agent 必须先向用户列出主库最新稳定版、主库最新测试版、个人仓库当前版本、建议目标版本及理由；只有用户明确确认完整版本号和 `alpha`/稳定通道后，才能修改版本或正式打包。
- 无法访问主库、Release 与 Tag 不一致、协作桥接版本尚未发布或版本归属不明确时，禁止猜测版本号，必须停止并请求用户确认。

#### 版本冲突保护

- Electron/npm 版本必须使用合法 SemVer `X.Y.Z-alpha.N`；`X.Y.Z.alpha.N` 仅可作为口头描述，不得写入版本文件或 Tag。
- 不得发布与主库或个人仓库已有 Tag 完全相同的版本。主库已发布 `X.Y.Z` 稳定版后，不得再发布 `X.Y.Z-alpha.N`。
- 从稳定版进入下一轮测试必须提升 patch，例如 `2.0.17 -> 2.0.18-alpha.0`。
- 已进入维护者提供的协作桥接版本时，以其 Commit、Tag、`package.json` 和 `package-lock.json` 为唯一基线；同一功能内部迭代只递增最后一位，例如 `2.0.17-alpha.0 -> 2.0.17-alpha.1`。
- 不得擅自补建、重命名或改变维护者确定的桥接版本、核心版本号和更新通道。

#### 打包与发布边界

- `npm run build`、专项测试和开发启动不属于正式打包，不要求修改版本号。
- 用户只说“打包”时，只生成个人仓库的本地安装包，不得自动创建 Tag、Release、上传产物或向主库提交版本变更。
- 用户只说“push”时，不得自动修改版本、打包或触发发布。
- 版本提交必须独立于功能提交；Tag、`package.json`、`package-lock.json` 和安装包版本必须完全一致。
